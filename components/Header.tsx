...
import { Phone, Mail, MapPin, Menu, X, ArrowRight, ShieldCheck, User } from "lucide-react";
    ...
    <header className="fixed top-0 left-0 w-full z-50 bg-surface/95 backdrop-blur-md shadow-[0_1px_8px_rgba(0,0,0,0.04)] border-b border-border-light">
      ...
      <div className="h-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-12 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex md:hidden items-center bg-slate-100 rounded px-1.5 py-0.5 border border-slate-200">
            <Link
              href={getSwitchLocaleUrl("en")}
              onClick={() => handleLocaleSwitch("en")}
              className={`px-1.5 py-0.5 text-[11px] font-bold rounded ${
                locale === "en" ? "bg-bronze text-white font-extrabold" : "text-slate-600"
              }`}
            >
              EN
            </Link>
            ...
            <Link
              ...
              onClick={() => handleLocaleSwitch("vi")}
              className={`px-1.5 py-0.5 text-[11px] font-bold rounded ${
                locale === "vi" ? "bg-bronze text-white font-extrabold" : "text-slate-600"
              }`}
            >
              VI
            </Link>
      ...
      {/* MOBILE NAV DRAWER */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-surface border-b border-border-light px-6 py-5 shadow-lg">
          <nav className="flex flex-col space-y-4 font-semibold text-on-surface text-base">
            <Link
              href={`/${locale}`}
